...
{
    AppLog::Error("%s","Failed to read randomness, aborting\n");
    std::abort();
}
...
static void ReportHardwareRand()
{
    ...
    if (g_rdseed_supported) {
        AppLog::Debug("%s", "Using RdSeed as additional entropy source\n");
    }
    if (g_rdrand_supported) {
        AppLog::Debug("%s", "Using RdRand as an additional entropy source\n");
    }
}
...
public:
    ...
    bool MixExtract(unsigned char* out, size_t num, CSHA512&& hasher, bool strong_seed) noexcept
    {
        ...
        bool ret;
        {
            ...
            // Finalize the hasher
            hasher.Finalize(buf);
            // Store the last 32 bytes of the hashBlock output as new RNG state.
            memcpy(m_state, buf + 32, 32);
        }
        // If desired, copy (up to) the first 32 bytes of the hashBlock output as output.
        if (num) {
            assert(out != nullptr);
            memcpy(out, buf, num);
        }
