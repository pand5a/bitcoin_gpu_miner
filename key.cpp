...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    uint256 hash;
    sha256::SHA256_CPU().Write((unsigned char*)str.data(), str.size()).Write(rnd, sizeof(rnd)).Finalize(hash.begin());
...
bool CKey::Derive(CKey& keyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
    ...
    if ((nChild >> 31) == 0) {
        ...
        Hash::BIP32Hash(cc, nChild, *pubkey.begin(), pubkey.begin()+1, vout.data());
    } else {
        assert(size() == 32);
        Hash::BIP32Hash(cc, nChild, 0, begin(), vout.data());
    }
