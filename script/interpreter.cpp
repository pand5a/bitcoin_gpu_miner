            ...
            switch (opcode)
            {
                ...
                case OP_HASH256:
                {
                    ...
                    if (opcode == OP_RIPEMD160)
                        ...
                        SHA256_CPU().Write(vch.data(), vch.size()).Finalize(vchHash.data());
                    else if (opcode == OP_HASH160)
                        Hash::CHash160().Write(vch.data(), vch.size()).Finalize(vchHash.data());
                    else if (opcode == OP_HASH256)
                        Hash::DoubleHash256().Write(vch.data(), vch.size()).Finalize(vchHash.data());
                    popstack(stack);
 ...
 *  required for the signature hashBlock done in-place
 */
template <class T>
...
public:
    ...
    void SerializeInput(S &s, unsigned int nInput) const {
        ...
        ::Serialize(s, txTo._vin[nInput]._previousOutput);
        ...
        if (nInput != nIn && (fHashSingle || fHashNone))
            ...
            ::Serialize(s, txTo._vin[nInput]._sequence);
    }
    ...
    void SerializeOutput(S &s, unsigned int nOutput) const {
        if (fHashSingle && nOutput != nIn)
            ...
            ::Serialize(s, txTo._vout[nOutput]);
    }
    ...
    void Serialize(S &s) const {
        // Serialize nVersion
        ::Serialize(s, txTo._nVersion);
        // Serialize vin
        unsigned int nInputs = fAnyoneCanPay ? 1 : txTo._vin.size();
        ::WriteCompactSize(s, nInputs);
        for (unsigned int nInput = 0; nInput < nInputs; nInput++)
             SerializeInput(s, nInput);
        // Serialize vout
        unsigned int nOutputs = fHashNone ? 0 : (fHashSingle ? nIn+1 : txTo._vout.size());
        ::WriteCompactSize(s, nOutputs);
        for (unsigned int nOutput = 0; nOutput < nOutputs; nOutput++)
             SerializeOutput(s, nOutput);
        // Serialize nLockTime
        ::Serialize(s, txTo._nLockTime);
    }
};
...
{
    Hash::CHashWriter ss(SER_GETHASH, 0);
    for (const auto& txin : txTo._vin) {
        ss << txin._previousOutput;
    }
    return ss.GetHash();
}
...
{
    Hash::CHashWriter ss(SER_GETHASH, 0);
    for (const auto& txout : txTo._vout) {
        ss << txout;
    }
    return ss.GetHash();
}
...
template PrecomputedTransactionData::PrecomputedTransactionData(const NetMessageTransaction& txTo);
//template PrecomputedTransactionData::PrecomputedTransactionData(const CMutableTransaction& txTo);

template <class T>
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    assert(nIn < txTo._vin.size());
    ...
    if (sigversion == SigVersion::WITNESS_V0) {
        ...
        } else if ((nHashType & 0x1f) == SIGHASH_SINGLE && nIn < txTo._vout.size()) {
            Hash::CHashWriter ss(SER_GETHASH, 0);
            ss << txTo._vout[nIn];
        ...
        ss << txTo._vin[nIn]._previousOutput;
        ss << scriptCode;
        ss << amount;
        ss << txTo._vin[nIn]._sequence;
...
bool GenericTransactionSignatureChecker<T>::CheckLockTime(const CScriptNum& nLockTime) const
{
    ...
    if (!(
        (txTo->_nLockTime <  LOCKTIME_THRESHOLD && nLockTime <  LOCKTIME_THRESHOLD) ||
        (txTo->_nLockTime >= LOCKTIME_THRESHOLD && nLockTime >= LOCKTIME_THRESHOLD)
    ))
        return false;
    ...
    if (nLockTime > (int64_t)txTo->_nLockTime)
        return false;
    ...
    if (TxIn::SEQUENCE_FINAL == txTo->_vin[nIn]._sequence)
        return false;
...
bool GenericTransactionSignatureChecker<T>::CheckSequence(const CScriptNum& nSequence) const
{
    ...
    const int64_t txToSequence = (int64_t)txTo->_vin[nIn]._sequence;

    // Fail if the transaction's version number is not set high
    // enough to trigger BIP 68 rules.
    if (static_cast<uint32_t>(txTo->_nVersion) < 2)
        return false;
...
// explicit instantiation
template class GenericTransactionSignatureChecker<NetMessageTransaction>;
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (witversion == 0) {
        if (program.size() == WITNESS_V0_SCRIPTHASH_SIZE) {
            ...
            SHA256_CPU().Write(&scriptPubKey[0], scriptPubKey.size()).Finalize(hashScriptPubKey.begin());
        ...
        ScriptError* serror)
{
    ...
    // scriptSig and _scriptPubKey must be evaluated sequentially on the same stack
    ...
    // Additional validation for spend-to-script-hashBlock transactions:
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash())
    {
        ...
        // P2SH  HASH <> EQUAL  _scriptPubKey would be evaluated with
